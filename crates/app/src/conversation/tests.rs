...
async fn persist_turn_provider_turns_expose_typed_canonical_records() {
    ...
    let user_record = records
        .iter()
        .find(|record| record.kind == crate::memory::CanonicalMemoryKind::UserTurn)
        .expect("expected canonical user turn record");
    let assistant_record = records
        .iter()
        .find(|record| record.kind == crate::memory::CanonicalMemoryKind::AssistantTurn)
        .expect("expected canonical assistant turn record");

    assert!(records.len() >= 2);
    assert_eq!(user_record.scope, crate::memory::MemoryScope::Session);
    assert_eq!(
        user_record.kind,
        crate::memory::CanonicalMemoryKind::UserTurn
    );
    assert_eq!(user_record.role.as_deref(), Some("user"));
    assert_eq!(user_record.content, "hello canonical memory");
    assert_eq!(user_record.session_id, "session-canonical-provider");

    assert_eq!(assistant_record.scope, crate::memory::MemoryScope::Session);
    assert_eq!(
        assistant_record.kind,
        crate::memory::CanonicalMemoryKind::AssistantTurn
    );
    assert_eq!(assistant_record.role.as_deref(), Some("assistant"));
    assert_eq!(assistant_record.content, "assistant-reply");
}
...
async fn persist_turn_explicit_acp_routing_exposes_typed_canonical_records() {
    ...
    let orchestrator = ConversationTurnCoordinator::new();
    let mut config = test_config();
    config.acp.enabled = true;
    config.acp.backend = Some(backend_id.to_owned());
