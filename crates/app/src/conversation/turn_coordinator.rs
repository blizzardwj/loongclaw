...
impl ConversationTurnCoordinator {
    ...
    pub async fn handle_turn(
        ...
        self.handle_turn_with_runtime_and_ingress(
            config, session_id, user_input, error_mode, &runtime, kernel_ctx, None,
        )
        .await
    }
    ...
    pub async fn handle_turn_with_ingress(
        ...
        let runtime = DefaultConversationRuntime;
        self.handle_turn_with_runtime_and_ingress(
            config, session_id, user_input, error_mode, &runtime, kernel_ctx, ingress,
        )
        .await
    }
    ...
    pub async fn handle_turn_with_runtime_and_ingress<R: ConversationRuntime + ?Sized>(
        ...
        let mut messages = runtime
            ...
            .await?;
        let ingress = ingress.filter(|value| value.has_contextual_hints());
        emit_turn_ingress_event(runtime, session_id, ingress, kernel_ctx).await;
        ...
        match provider_result {
            Ok(turn) => {
                ...
                let turn_result = if should_use_safe_lane_plan_path(config, &lane_decision, &turn) {
                    ...
                    .execute_turn_with_ingress(&turn, kernel_ctx, ingress)
                    .await
                };
