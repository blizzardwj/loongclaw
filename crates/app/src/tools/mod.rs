...
pub fn canonical_tool_name(raw: &str) -> &str {
    match raw {
        ...
        other => {
            #[cfg(feature = "feishu-integration")]
            if let Some(canonical) = feishu::canonical_feishu_tool_name(other) {
                return canonical;
            }
            other
        }
...
pub fn is_known_tool_name(raw: &str) -> bool {
    match canonical_tool_name(raw) {
        ...
        other => {
            #[cfg(feature = "feishu-integration")]
            {
                feishu::is_known_feishu_tool_name(other)
            }
...
pub fn capability_snapshot() -> String {
    capability_snapshot_with_config(Some(runtime_config::get_tool_runtime_config()))
}

pub(crate) fn capability_snapshot_with_config(
    config: Option<&runtime_config::ToolRuntimeConfig>,
) -> String {
    let entries = tool_registry_with_config(config);
...
mod tests {
    ...
    fn capability_snapshot_is_deterministic() {
        let snapshot =
            capability_snapshot_with_config(Some(&runtime_config::ToolRuntimeConfig::default()));
    ...
    fn tool_registry_returns_all_known_tools() {
        let entries =
            tool_registry_with_config(Some(&runtime_config::ToolRuntimeConfig::default()));
