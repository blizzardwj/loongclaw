...
async fn run_feishu_send_cli(
    config_path: Option<&str>,
    account: Option<&str>,
    request: &mvp::channel::FeishuChannelSendRequest,
) -> CliResult<()> {
    mvp::channel::run_feishu_send(config_path, account, request).await
}
...
mod cli_tests {
    ...
    #[test]
    fn feishu_auth_start_subcommand_parses_nested_namespace() {
        let cli = Cli::try_parse_from([
            "loongclaw",
            "feishu",
            "auth",
            "start",
            "--redirect-uri",
            "http://127.0.0.1:34819/callback",
            "--scope",
            "offline_access",
            "--capability",
            "message-write",
        ])
        .expect("nested feishu auth start should parse");

        match cli.command {
            Some(Commands::Feishu {
                command: feishu_cli::FeishuCommand::Auth { command },
            }) => match command {
                feishu_cli::FeishuAuthCommand::Start(options) => {
                    assert_eq!(
                        options.redirect_uri,
                        "http://127.0.0.1:34819/callback".to_owned()
                    );
                    assert_eq!(options.scopes, vec!["offline_access".to_owned()]);
                    assert_eq!(
                        options.capabilities,
                        vec![feishu_support::FeishuAuthCapability::MessageWrite]
                    );
                    assert!(!options.include_message_write);
                }
                other => panic!("unexpected feishu auth command parsed: {other:?}"),
            },
            other => panic!("unexpected command parsed: {other:?}"),
        }
    }
    ...
    #[test]
    fn feishu_messages_search_subcommand_parses_open_id_and_filters() {
        let cli = Cli::try_parse_from([
            "loongclaw",
            "feishu",
            "search",
        ...
        .expect("nested feishu messages search should parse");

        match cli.command {
            Some(Commands::Feishu {
                command: feishu_cli::FeishuCommand::Search { command },
            }) => {
                let feishu_cli::FeishuSearchCommand::Messages(options) = command;
                assert_eq!(options.grant.open_id.as_deref(), Some("ou_123"));
                assert_eq!(options.query, "incident".to_owned());
                assert_eq!(options.chat_ids, vec!["oc_456".to_owned()]);
            }
            other => panic!("unexpected command parsed: {other:?}"),
        }
    }
